...
namespace minLang{
parseRes<minLang::ast::call> parseCall(std::span<const mediumToken> tokens){
	if(tokens.size() < 2)
		return std::nullopt;
	...
	std::vector<minLang::ast::expr> callArgs;
	while(!tList.empty()){
	...
	return makeParseRes(minLang::ast::call{name, callArgs}, 2);
}

parseRes<minLang::ast::call> parseGetCallOp(std::span<const mediumToken> tokens){
	if(tokens.size() < 2)
		return std::nullopt;
	if(!std::holds_alternative<basicToken>(tokens.front().value))
		return std::nullopt;
	...
	std::span<const mediumToken> tList(list.value);
	std::vector<minLang::ast::expr> callArgs{name};
	while(!tList.empty()){
		auto argTry = parseExpr(tList);
		if(!argTry)
			return std::nullopt;
	...
	return makeParseRes(minLang::ast::call{"get", callArgs}, 2);
}

parseRes<minLang::ast::expr> parseExpr_noOperators(std::span<const mediumToken> tokens){
	//try literal
	{
		const auto& literalTry = parseLiteral(tokens);
		if(literalTry){
			return makeParseRes<minLang::ast::expr>(literalTry->val, literalTry->toksConsumed);
		}
	}
	...
	//try function call
	{
		...
		if(callTry){
			return makeParseRes<minLang::ast::expr>(callTry->val, callTry->toksConsumed);
		}
	}
	...
	//try get call op
	{
		...
		if(getTry){
			return makeParseRes<minLang::ast::expr>(getTry->val, getTry->toksConsumed);
		}
	}
	...
	//try var name
	{
		...
		if(!std::holds_alternative<basicToken>(tokens.front().value))
			return std::nullopt;
		const std::string& name = std::get<basicToken>(tokens.front().value).val;
		return makeParseRes(minLang::ast::expr(name), 1);
	}
...
struct exprOperatorList{
	std::vector<minLang::ast::expr> exprs;
	...
	exprOperatorList(const minLang::ast::expr& exp) : exprs{exp}{};
	void addExpr(unsigned int opNum, const minLang::ast::expr& exp){
		exprs.push_back(exp);
		betweenExprOperators.push_back(opNum);
	};
	//given a + b * c + d replace with a + (b * c) + d
	void replaceExprsAndOp(unsigned int firstExprIndex, const minLang::ast::expr& newExpr){
	...
	void replacePairsWithCall(unsigned int operaterPrescNum, const std::string& callName){
		for(unsigned int i=0;i<betweenExprOperators.size();i++){
			if(betweenExprOperators[i] == operaterPrescNum){
				const auto& newExpr = minLang::ast::call{callName, {exprs[i], exprs[i+1]}};
...
parseRes<minLang::ast::expr> minLang::parseExpr(std::span<const mediumToken> tokens){
		...
		if(i == getTwoCharPrescedenceNum('!', '=')){
			for(unsigned int i=0;i<opList.betweenExprOperators.size();i++){
				if(opList.betweenExprOperators[i] == getTwoCharPrescedenceNum('!', '=')){
					const auto& eqCall = minLang::ast::call{"equal", {opList.exprs[i], opList.exprs[i+1]}};
					const auto& notCall = minLang::ast::call{"not", {eqCall}};
...
parseRes<std::vector<minLang::ast::expr>> minLang::parseCommaSeperatedExprList(std::span<const mediumToken> tokens){
	if(tokens.empty())
		return makeParseRes(std::vector<minLang::ast::expr>(), 0);

	std::vector<minLang::ast::expr> output;
