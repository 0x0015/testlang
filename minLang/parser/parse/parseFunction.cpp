...
parseRes<minLang::ast::function> minLang::parseFunction(std::span<const mediumToken> tokens){
	//type
	...
	return makeParseRes(minLang::ast::function{ty->val, name, args->val, body->val}, offset);
}

//mostly copied from parseFunction
parseRes<minLang::ast::function> minLang::parseExternalFunction(std::span<const mediumToken> tokens){
	//extern
	if(tokens.empty())
		return std::nullopt;
	if(!std::holds_alternative<basicToken>(tokens.front().value))
	...
	return makeParseRes(minLang::ast::function{ty->val, name, args->val, {}, minLang::ast::function::positionStatus::external}, offset);
}

parseRes<std::vector<minLang::ast::function::argument>> minLang::parseFunctionArgs(std::span<const mediumToken> tokens){
	if(tokens.empty())
		return std::nullopt;
	...
	if(list.type != mediumToken::tokList::type_t::PAREN)
		return std::nullopt;
	std::vector<minLang::ast::function::argument> output;
	...
	for(unsigned int i=0;i<list.value.size();i++){
		...
		output.push_back(minLang::ast::function::argument{ty->val, name.val});
	}

	return makeParseRes(output, 1);
}

parseRes<std::monostate> minLang::looksLikeFunction(std::span<const mediumToken> tokens){
	//type
	auto ty = parseType(tokens);
	int offset = 0;
	if(ty){
...
//mostly copied from parseFunction
parseRes<std::monostate> minLang::looksLikeExternalFunction(std::span<const mediumToken> tokens){
	//extern
	if(tokens.empty())
		return std::nullopt;
	if(!std::holds_alternative<basicToken>(tokens.front().value))
