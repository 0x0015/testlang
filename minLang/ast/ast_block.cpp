...
void minLang::ast::block::dump() const{
	std::cout<<"{"<<std::endl;
...
minLang::ast::block minLang::ast::block::clone() const{
	minLang::ast::block output{statements};
	for(auto& state : output.statements){
		if(std::holds_alternative<declaration>(state)){
			...
			ifState.ifBody = std::make_shared<minLang::ast::block>(ifState.ifBody->clone());
			ifState.elseBody = std::make_shared<minLang::ast::block>(ifState.elseBody->clone());
		}else if(std::holds_alternative<forStatement_normal>(state)){
			...
			forState.body = std::make_shared<minLang::ast::block>(forState.body->clone());
		}else if(std::holds_alternative<returnStatement>(state)){
